...
import { RouteNode, convertRoutes, createFilesFromRoutes } from "./generate/routegen.js";
...
async function openapiTS(
  ...
  let pathSpec: Record<string, RouteNode> = {};
  const output: string[] = [];
  ...
  for (const k of Object.keys(rootTypes)) {
    if (rootTypes[k] && !EMPTY_OBJECT_RE.test(rootTypes[k])) {
      ...
      output.push(
        options.exportType
          ? `export type ${k}${typeArguments} = ${rootTypes[k]};`
...
`);
  if (paths) {
    ...
    // And now, create the autogenerated filetree.
    pathSpec = convertRoutes(paths);
  }
  ...
  return {
    contract: output.join("\n"),
    pathSpec,
  };
}

export default openapiTS;
export { createFilesFromRoutes };

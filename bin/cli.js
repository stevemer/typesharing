...
Options
  ...
  --installPath, -i            Specify installation path if routers should be autogenerated (default: disabled)
  --schemaPathMapping -s       Specify schema path mapping (e.g., @schema) if routers are autogenerated (required if --installPath set)
...
if (
  (args.includes("-i") || args.includes("--installPath")) &&
  !(args.includes("-s") || args.includes("--schemaPathMapping"))
)
  errorAndExit(`If installPath is set, schemaPathMapping must be set as well.`);
...
async function generateSchema(pathToSpec) {
  ...
  if (output === OUTPUT_FILE) {
    ...
    fs.writeFileSync(outputFilePath, result.contract, "utf8");

    // Now, genreate the rest.
    console.dir(result.pathSpec, { depth: null });
    createFilesFromRoutes(result.pathSpec, flags.installPath, flags.schemaPathMapping);
...
async function main() {
  ...
  const pathToSpec = flags._.sort()[0];
